...
addText(fig,[0.10 0.24 0.80 0.11], ...
    "Traditional planning reacts to position. PathWeaver scores time-aligned future occupancy risk.", ...
...
evaluationFile=fullfile(root,'artifacts','evaluation','evaluation_results.csv');
if options.refreshEvaluation || ~isfile(evaluationFile)
    runPathWeaverEvaluation(outputDirectory=fullfile(root,'artifacts','evaluation'));
end
results=readtable(evaluationFile,'TextType','string');
fig=newSlide(colors); heading(fig,"MEASURED V0.1 RESULTS — 10 PAIRED SEEDS");
base=results(results.mode=="baseline",:); risk=results(results.mode=="risk-aware",:);
facts={sprintf('COMPLETION\n%d / %d',sum(risk.completed),height(risk)), ...
    sprintf('COLLISIONS\n%d',sum(risk.collision)), ...
    sprintf('MEAN MIN TTC\n%.3f s',mean(risk.minimumTtcS)), ...
    sprintf('MEAN LATENCY\n%.2f ms',mean(risk.averagePlannerLatencyS)*1000)};
for k=1:4
    x=0.055+(k-1)*0.235;
...
comparison=sprintf(['Baseline mean minimum TTC: %.3f s   |   Risk-aware: %.3f s\n' ...
    'Preliminary simulation evidence — not road-deployment validation'], ...
    mean(base.minimumTtcS),mean(risk.minimumTtcS));
addText(fig,[0.09 0.20 0.82 0.14],comparison,16,colors.silver,'center','normal');
saveSlide(fig,outputDirectory,'06-results.png');

fig=newSlide(colors); heading(fig,"STATEFLOW BEHAVIOUR — VERIFIED DECISIONS");
...
addText(fig,[0.10 0.14 0.80 0.09], ...
    "Programmatically generated chart • seven direct decisions matched MATLAB logic", ...
